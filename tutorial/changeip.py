...
class IP_Handler:
    ...
    def DialBroadband(self):
        ...
        try:
            ...
            handle, result = IP_Handler.Connect(dialname, account, passwd)
            if result == 0:
    ...
    def ChangeIP(self):
        data = IP_Handler.Check_for_Broadband()
        data = IP_Handler.Check_for_Broadband()
        # if exist running broadband connection, disconnected it.
        if data != None:
            for p in data:
                IP_Handler.ShowIpAddress(p[0])
                if (IP_Handler.Disconnect(p[0]) == "success"):
                    print("%s has been disconnected." % p[1])
                time.sleep(3)
        pid, res = IP_Handler.DialBroadband()
        IP_Handler.ShowIpAddress(pid)
        time.sleep(3)
        return "IP change success"
